...
PRIVATE data directory: wherever `$BRAIN_DB` points (default `~/Claude/brain-db`).
That directory may be a plain folder or a git repo. You operate the data THROUGH
this system: read/write it under `$BRAIN_DB`, and if it is a git repo, commit it
THERE. This file is how you operate the brain. (The owner's profile is
`$BRAIN_DB/PROFILE.md`.)

## Session start: load context, cheaply

1. Read `$BRAIN_DB/CRITICAL_FACTS.md` (~120 tokens -- always).
2. Read `$BRAIN_DB/PROFILE.md` (who the owner is, how they want you to work).
   This is why the brain exists: open every session already knowing them. Never
   make the owner re-explain themselves.
...
brain's markdown) is how you retrieve:
- `python3 db/query.py search "<terms>" [--context X] [--raw] [--limit N]` --
  ...
  `--context research`), or `--raw` to search the raw corpus.
...
pattern-matching. (If a source pre-tags its raw material mechanically, treat
those tags as hints only, and recompile with judgment.)

## Context is a dimension (not separate vaults)

Tag pages with `context/<name>` (e.g. `context/work`, `context/research`,
`context/personal`). A "context" is a scoped VIEW + a
...
- Do NOT bulk-import old vaults, archives, or stale documents. Bring content in
  only when the owner directs, current/relevant only.
